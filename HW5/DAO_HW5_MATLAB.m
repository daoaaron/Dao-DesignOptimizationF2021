...
df = @(x)[2*x(1) , 2*(x(2)-3)]; % ROW. Each new column is a new variable, each new row is a new function.

g = @(x)[x(2)^2-2*x(1); (x(2)-1)^2+5*x(1)-15];  % Equations for all gs!
dg = @(x)[-2 2*x(2); 5 2*(x(2)-1)] ;  % Equations for all dgs!
